			  ...
			  { 
				  ...
				  else if (t.getKind() == Kind.IDENT)
				  {
					  ...
					  if (isKind(Kind.LSQUARE)) {
							consume();
							Expr x = expr();
							x.setType(Type.INT); //manually set type here
							match(Kind.COMMA);
							Expr y = expr();
							y.setType(Type.INT);
							match(Kind.RSQUARE);
			...
			if (t.getKind() == Kind.IDENT)
			{
				...
				if(t.getKind() != Kind.SEMI)
				{
					...
					if (t.getKind() == Kind.LARROW) {
						...
						PixelSelector p = null;
						Expr e = expr();
						ReadStatement r = new ReadStatement(t2, n.getName(), p, e);
						decsAndStatements.add(new VarDeclaration(t2, n, var, e));
